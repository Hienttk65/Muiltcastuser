    ...
    char* argv[])
{
    ...
    outdata.open("result_VRCast.txt"); // opens the file
        ...
        try {
            ...
            for (int m = 0; m < 15; m++)
            {
                for (int n = 0; n < 24; n++)
                {
                    ...
                    for (int k = 0; k < 5; k++)
                    {
                        if (int(x_nk[n][k].get(GRB_DoubleAttr_X)) == 1)
                            V_mn[m][n] = k;
                    }
                    VQ = VQ + (w_mn[m][n] * Q_nk[V_mn[m][n]]);
                }
